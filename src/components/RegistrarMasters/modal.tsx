...
const RegistrarMasterModal:FC<RegistrarMastersModalProps & {toggleModal: (value: RegistrarMastersModalProps) => void}> = (props) => {
    return (
        <Modal opened={props.status} onClose={() => props.toggleModal({status: false, type: 'Create'})} centered size="xl" withCloseButton={true}  title={props.type === "Edit" ? "Edit Registrar Master" : "Create Registrar Master"} overlayProps={{
